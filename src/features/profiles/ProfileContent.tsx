...
export default function ProfileContent({ profile }: { profile: Profile }) {
    ...
    const items = [
        ...
        { key: 'events', label: `Events`, description: `View events ${profile.displayName} is hosting or attending`, icon: ListBulletIcon },
        { key: 'members', label: `Members`, description: `View other members of this app`, icon: UserGroupIcon },
    ]

    const selected = items.find(i => i.key === selectedItem) || items[0];
        ...
        <div className="flex w-full gap-3 h-[64vh]">
            <ul className="list bg-base-100 rounded-box w-1/4 pt-2">
                {items.map(({ key, label, description, icon: Icon }) => (
                    <li onClick={() => setSelectedItem(key)} key={key}
